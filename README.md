...
API: `GET /api/gallery` (returns `{ items, people }` with curated event categories and exact duplicates removed) · `GET /api/label-audit` (downloads a complete picture-label checklist) · `POST /api/upload` (multipart: token, photo/video file, cat, label, cap; photos ≤20 MB, videos ≤500 MB) · `POST /api/delete` (JSON: token, id) · `POST /api/subscribe` (JSON: email).
